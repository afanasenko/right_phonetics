<?php 
	...
	{
		$cue_pattern = rand() % 2 ;
		$antipattern = $cue_pattern == 1 ? 0 : 1;
	
		$query = 'SELECT * FROM `vowel_duration` WHERE `long_vowel`=' . $cue_pattern . ' ORDER BY RAND() LIMIT 1;';
		
		$res1 = mysql_query($query);
		
		$query = 'SELECT * FROM `vowel_duration` WHERE `long_vowel`=' . $antipattern . ' ORDER BY RAND() LIMIT 3;';
	...
	{
		$res = mysql_query('SELECT `long_vowel` FROM `vowel_duration` WHERE `id`=' . $word_id . ';');
